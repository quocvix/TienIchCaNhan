...
export default function GameRoom() {
    ...
    const handleConfirmPlayers = (newPlayers: string[]) => {
        ...
        activePlayers.forEach((name, idx) => {
            mockRound[name] = mockScores[idx] !== undefined ? mockScores[idx] : 0;
        });
            ...
            <div className="flex-1 p-4 mt-2 overflow-y-auto">
                {!hasPlayers ? (
                    ...
                    <div className="border border-dashed border-white/10 rounded-[2rem] p-8 flex flex-col items-center justify-center gap-6 bg-gradient-to-b from-white/[0.02] to-transparent mt-4">
                        ...
                        <Drawer>
                            ...
                            <DrawerContent className="bg-[#0a0a0a] border-white/5 outline-none overflow-hidden p-0">
                                <AddPlayerDrawer onConfirm={handleConfirmPlayers} />
                            </DrawerContent>
                        </Drawer>
                    </div>
                ) : (
                    ...
                    <div className="flex flex-col gap-6">
                        {/* Player Totals */}
                        <div className={`grid gap-2 ${players.length === 2 ? 'grid-cols-2' : players.length === 3 ? 'grid-cols-3' : 'grid-cols-4'}`}>
                            {players.map((name, idx) => {
                                const total = getPlayerTotal(name);
                                return (
                                    <div key={idx} className="bg-[#151517] rounded-xl flex flex-col items-center justify-center py-4 gap-2 shadow-sm">
                                        <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                                            {name}
                                        </span>
                                        <span className={`text-[22px] font-black ${total > 0 ? 'text-emerald-500' : total < 0 ? 'text-[#ff3333]' : 'text-gray-400'}`}>
                                            {total > 0 ? `+${total}` : total}
                                        </span>
                                    </div>
                                );
                        ...
                        <div className="flex flex-col gap-3 px-1 mt-4">
                            {history.map((round, roundIdx) => (
                                <div key={roundIdx} className="flex items-center">
                                    <span className="w-8 text-xs font-bold text-gray-500">{roundIdx + 1}</span>
                                    <div className={`flex-1 grid gap-2 ${players.length === 2 ? 'grid-cols-2' : players.length === 3 ? 'grid-cols-3' : 'grid-cols-4'}`}>
                                        {players.map((name, sIdx) => {
                                            const score = round[name] || 0;
                                            return (
                                                <div key={sIdx} className="flex justify-center">
                                                    <span className={`text-[15px] font-bold ${score > 0 ? 'text-emerald-500' : score < 0 ? 'text-[#ff3333]' : 'text-gray-400'}`}>
                                                        {score > 0 ? `+${score}` : score}
                                                    </span>
                                                </div>
                                            );
                ...
                {!hasPlayers ? (
                    ...
                    <Drawer>
                        <DrawerTrigger asChild>
                            ...
                            >
                                <Plus size={24} className="mr-2" strokeWidth={2.5} />
                                GHI ĐIỂM
                            </Button>
                        </DrawerTrigger>
                        <DrawerContent className="bg-[#0a0a0a] border-white/5 outline-none overflow-hidden p-0">
                            <ScoreDrawer players={players} onConfirm={handleConfirmScore} />
                        </DrawerContent>
                    </Drawer>
                )}
            </div>
