...
const createEmptyRoundInput = (count: number): string[] => Array(count).fill("");
...
const loadInitialState = () => {
  ...
  try {
    ...
    const savedPlayers: Player[] = Array.isArray(parsed.players) ? parsed.players : createPlayers(DEFAULT_PLAYER_COUNT);
...
export default function ScoreTrackerFlexiblePlayers() {
  const [players, setPlayers] = useState<Player[]>(() => loadInitialState().players);
  const [rounds, setRounds] = useState<Round[]>(() => loadInitialState().rounds);
  const [currentRound, setCurrentRound] = useState<string[]>(() => loadInitialState().currentRound);
  const [editingRoundId, setEditingRoundId] = useState<string | null>(() => loadInitialState().editingRoundId);
  const [editingScores, setEditingScores] = useState<string[]>(() => loadInitialState().editingScores);
