...
export default function AnPhatHeoPanel({
    ...
    return (
        <div className="flex flex-col gap-0 w-full">
            {/* Header */}
            <div className="flex items-center pb-2.5 mb-1 border-b border-white/5 text-[11px] font-bold uppercase tracking-wider">
                <div className="w-16 shrink-0 text-gray-500 pr-2">
                    Người chơi
                </div>
                <div className="flex-1 grid grid-cols-2 gap-2 pl-3 items-center">
                    <div className="text-emerald-500">
                        Ăn
                    </div>
                    <div className="text-red-400 text-right pr-1">
                        Phạt
                    </div>
                </div>
            </div>
            ...
            {players.map((name) => {
                ...
                return (
                    <div
                        key={name}
                        className="flex items-center py-2.5 border-t border-white/[0.04] first:border-t-0"
                    >
                        {/* Player name on the left */}
                        <div className="w-16 shrink-0 text-sm font-bold text-gray-300 truncate pr-2 border-r border-white/5 flex items-center">
                            {name}
                        </div>

                        {/* Controls split into columns: Left (Ăn), Right (Phạt) */}
                        <div className="flex-1 grid grid-cols-2 gap-2 pl-3 items-center">
                            {/* Column Ăn */}
